...
def startGame(request):
    if request.method == 'POST':
        ...
        response = requests.post(url='http://backend:8080/start', data={'username': username, 'game': 'start'})
...
def make_move(request):
    if request.method == 'POST':
        ...
        response = requests.post(url='http://backend:8080/make_move', data={'username': username, 'move': move, 'finish': finish})
